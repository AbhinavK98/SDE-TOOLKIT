...
Airplanes talking directly to each other creates N² connections. Adding a flight requires updating every plane.
...
**Air traffic control tower** — planes don't radio each other; they go through the tower.
...
| File | Purpose |
|------|--------|
| `with_pattern.py` | ControlTower, Airplane, full demo |
...
| Bad | Good |
|-----|------|
| Airplane holds references to all other airplanes | ControlTower mediates; airplanes send/receive via tower only |


## Code Walkthrough

**Key classes:** `AirTrafficControl, ControlTower, Airplane`
...
Mediator Pattern teaches you to structure objects so systems grow without breaking. Master the **intent**, draw the **diagram**, then read the **code**.
...
- **Key classes:** Observer, event buses
- **Run:** `Observer, event buses`
