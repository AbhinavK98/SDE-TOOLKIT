# Singleton Pattern — Interview Q&A

---

## Core Questions

### 1. What problem does Singleton Pattern solve?

**Answer:** Multiple Logger instances write to different files or duplicate config. Some resources must exist exactly once.

---

### 2. Explain the real-world analogy.

**Answer:** **President of a country** — only one at a time. Elections return the same office, not a new country.

---

### 3. Walk through the code in this folder.

**Answer:** Key classes: `Logger`. Flow: Logger('a') → Logger('b') → same instance → shared log_count.
...
> **Multiple Logger instances write to different files or duplicate config. Some resources must exist exactly once....** This folder shows the fix.
