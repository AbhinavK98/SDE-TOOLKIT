...
Restaurant must serve consistent cuisine families — North Indian starter+main+dessert together. One factory per cuisine.
...
**IKEA room sets** — pick Scandinavian vs Industrial; get matching sofa, table, lamp from one factory.
...
| File | Purpose |
|------|--------|
| `bad_example.py` | Monolithic cuisine if/elif |
| `good_example.py` | Abstract factory per cuisine family |
...
**Key classes:** `Starter, MainCourse, Dessert, CuisineFactory, NorthIndianCuisine`
...
```bash
cd "13. Abstract Factory Pattern"
...
Abstract Factory Pattern teaches you to structure objects so systems grow without breaking. Master the **intent**, draw the **diagram**, then read the **code**.
