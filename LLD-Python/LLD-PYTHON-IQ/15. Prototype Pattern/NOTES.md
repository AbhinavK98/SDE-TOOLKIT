...
Creating complex ChessBoard from scratch is expensive. Copying shares references — moving a piece on copy moves original.
...
**Photocopy vs snapshot** — shallow copy shares sticky notes; deep copy gives independent pages.
...
| File | Purpose |
|------|--------|
| `bad_Example.py` | Shared piece references |
| `good_example.py` | ChessBoard.clone() deep copy |
| `deep_copyandshallow_copy.py` | Standalone deep vs shallow demo |
...
**Key classes:** `ChessPiece, ChessBoard`
...
Prototype Pattern teaches you to structure objects so systems grow without breaking. Master the **intent**, draw the **diagram**, then read the **code**.
