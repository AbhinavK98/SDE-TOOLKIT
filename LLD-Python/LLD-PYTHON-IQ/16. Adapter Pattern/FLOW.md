# Adapter Pattern — Execution Flow
...
| 4 | OrderService(adapter) → create_order → adapter.send() → send_grid.send_email() |
...
**Flow:** OrderService(adapter) → create_order → adapter.send() → send_grid.send_email()
