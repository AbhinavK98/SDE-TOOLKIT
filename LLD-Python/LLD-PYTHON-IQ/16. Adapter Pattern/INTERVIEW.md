...
**Answer:** SendGrid API uses send_email(recipient, subject, content) but your app expects send(to, title, body).

---

### 2. Explain the real-world analogy.

**Answer:** **Power plug adapter** — US plug into EU socket. Same electricity, different interface.

---

### 3. Walk through the code in this folder.

**Answer:** Key classes: `NotificationService, SendGridAdapter, SendGridEmailService, OrderService`. Flow: OrderService(adapter) → create_order → adapter.send() → send_grid.send_email().
...
| Bad | Good |
|-----|------|
| OrderService directly calls incompatible SendGrid API | SendGridAdapter implements NotificationService, wraps SendGrid |
...
> **SendGrid API uses send_email(recipient, subject, content) but your app expects send(to, title, body)....** This folder shows the fix.
