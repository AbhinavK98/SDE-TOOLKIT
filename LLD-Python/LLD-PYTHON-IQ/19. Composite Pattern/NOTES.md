...
File system has files and folders; folders contain files and subfolders. Client shouldn't treat them differently.
...
**Organization chart** — CEO manages VPs and individual contributors through same report structure.
...
| File | Purpose |
|------|--------|
| `bad_example.py` | Flat folder — files only |
| `good_example.py` | Nested folder tree |
...
**Key classes:** `FileSystemComponent, File, Folder`
...
Composite Pattern teaches you to structure objects so systems grow without breaking. Master the **intent**, draw the **diagram**, then read the **code**.
...
- **Key classes:** Tree structures, Iterator
- **Run:** `Tree structures, Iterator`
