...
Every new payment type means editing `PaymentProcessor` with another `elif` — open for modification, closed for extension.
...
A **power strip** accepts new plugs without rewiring the wall. Your processor should accept new payment plugins without internal surgery.
...
```
2. SOLID Principles/2. Open Close Principle (OCP)/
├── 1. bad_example.py
├── 2. good_example.py
├── NOTES.md
├── FLOW.md
...
| File | Purpose |
|------|--------|
| `1. bad_example.py` | String-based branching in processor |
| `2. good_example.py` | Strategy-like payment hierarchy |
...
**Key classes:** `PaymentProcessor, PaymentMethod, UPIPayment, DebitCardPayment, CreditCardPayment`
...
Open/Closed Principle (OCP) teaches you to structure objects so systems grow without breaking. Master the **intent**, draw the **diagram**, then read the **code**.
...
- **Key classes:** Strategy Pattern, Factory Pattern
- **Run:** `Strategy Pattern, Factory Pattern`
