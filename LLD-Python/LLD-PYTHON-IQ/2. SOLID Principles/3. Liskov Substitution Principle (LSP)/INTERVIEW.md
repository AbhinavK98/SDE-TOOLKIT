# Liskov Substitution Principle (LSP) — Interview Q&A

---

## Core Questions

### 1. What problem does Liskov Substitution Principle (LSP) solve?

**Answer:** Subclass breaks parent contract — `FixedDepositAccount.withdraw()` raises Exception. Code expecting `BankAccount` crashes.

---

### 2. Explain the real-world analogy.

**Answer:** Every **bird** should fly. If **Penguin** extends Bird but can't fly, the abstraction is wrong. Split `FlyableBird` from `Bird`.

---

### 3. Walk through the code in this folder.

**Answer:** Key classes: `Account, WithdrawableAccount, SavingsAccount, FixedDepositAccount`. Flow: main → FixedDepositAccount.deposit(1000).
...
> **Subclass breaks parent contract — `FixedDepositAccount.withdraw()` raises Exception. Code expecting `BankAccount` crashe...** This folder shows the fix.
