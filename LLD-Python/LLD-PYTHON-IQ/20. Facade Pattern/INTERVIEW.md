...
**Answer:** Mobile app calls login, profile, orders from three services — client knows too much orchestration.

---

### 2. Explain the real-world analogy.

**Answer:** **Hotel concierge** — one request: arrange dinner, spa, taxi. You don't call each vendor.

---

### 3. Walk through the code in this folder.

**Answer:** Key classes: `UserService, OrderService, ApiGateway`. Flow: api_gateway.get_all_details() → login → profile → orders.
...
> **Mobile app calls login, profile, orders from three services — client knows too much orchestration....** This folder shows the fix.
