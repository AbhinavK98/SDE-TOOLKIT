...
Waiter shouldn't know how to cook pizza vs burger. Order placement and execution must be decoupled for queuing/undo.
...
**Restaurant order ticket** — waiter writes order; chef executes later. Ticket is the command object.
...
| File | Purpose |
|------|--------|
| `order.py` | Command interface execute() |
| `pizza_order.py` | Command wrapping chef pizza |
| `burger_order.py` | Command wrapping chef burger |
| `chef.py` | Receiver — cook methods |
| `waiter.py` | Invoker — take_order |
| `main.py` | Waiter takes burger order |
...
**Key classes:** `Order, PizzaOrder, BurgerOrder, Chef, Waiter`
...
Command Pattern teaches you to structure objects so systems grow without breaking. Master the **intent**, draw the **diagram**, then read the **code**.
...
- **Key classes:** Memento undo, job queues
- **Run:** `Memento undo, job queues`
