...
CSV and JSON parsers share open-parse-close steps but differ only in parsing. Duplicating the skeleton violates DRY.
...
**McDonald's franchise** — same steps (prep, cook, serve); only the recipe (parse logic) varies per location.
...
| File | Purpose |
|------|--------|
| `pattern.py` | DataParser ABC + CSVParser + demo |
...
**Key classes:** `DataParser, CSVParser`
...
Template Method Pattern teaches you to structure objects so systems grow without breaking. Master the **intent**, draw the **diagram**, then read the **code**.
...
- **Key classes:** Hollywood Principle — don't call us, we'll call you
- **Run:** `Hollywood Principle — don't call us, we'll call you`
