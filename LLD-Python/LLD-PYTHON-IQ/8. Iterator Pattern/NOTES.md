...
**Spotify playlist** — press next regardless of how songs are stored internally.
...
| File | Purpose |
|------|--------|
| `pattern.py` | List-backed PlaylistIterator |
| `pattern_using_linked_list.py` | Linked-list backed iterator |
...
**Key classes:** `Song, Playlist, PlaylistIterator, Node`
...
Iterator Pattern teaches you to structure objects so systems grow without breaking. Master the **intent**, draw the **diagram**, then read the **code**.
