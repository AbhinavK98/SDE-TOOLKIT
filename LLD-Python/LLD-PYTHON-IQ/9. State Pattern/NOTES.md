...
Transport app with giant if/elif for bike vs walk vs car modes — adding metro means editing every method.
...
**Traffic light** — same intersection, behavior changes by state (red/yellow/green) without rewriting the intersection.
...
| File | Purpose |
|------|--------|
| `transport_mode.py` | State interface eta(), directions() |
| `bike_mode.py` | Bike state implementation |
| `walk_mode.py` | Walk state implementation |
| `transport_Service.py` | Context with set_mode() |
| `main.py` | BikeMode demo |
...
**Key classes:** `TransportMode, BikeMode, WalkMode, TransportService`
...
State Pattern teaches you to structure objects so systems grow without breaking. Master the **intent**, draw the **diagram**, then read the **code**.
...
- **Key classes:** Strategy (similar structure, different intent)
- **Run:** `Strategy (similar structure, different intent)`
